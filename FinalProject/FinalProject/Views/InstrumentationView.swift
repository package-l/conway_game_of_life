        ...
        WithViewStore(store) { viewStore in
            HStack {
                Slider(
                    ...
                    minimumValueLabel: Text("5").font(.system(.subheadline, design: .rounded)).foregroundColor(Color(.white)),
                    maximumValueLabel: Text("40").font(.system(.subheadline, design: .rounded)).foregroundColor(Color(.white)),
                    label: { Spacer() }
                )
                .frame(width: width * 0.65, alignment: .bottomLeading)
        ...
        WithViewStore(store) { viewStore in
            HStack {
                ...
                Text("Simulation")
                    ...
                    .padding(0.0)
                    .foregroundColor(viewStore.isRunningTimer ? .red : Color("textColor")) //Color("accent")
            }
        }
    }
            ...
            HStack {
                Slider(
                    ...
                    minimumValueLabel: Text("0.0")
                        .font(.system(.subheadline, design: .rounded)).foregroundColor(Color(.white)),
        ...
        WithViewStore(store) { viewStore in
            HStack {
                ...
                ThemedButton(text: "Step") {
                    //viewStore.send(.setAtStartTrue)
                    withAnimation(
                        .linear(duration: 3.0)
                    ) {
                        viewStore.send(.startAnimation)
                    }
                }
